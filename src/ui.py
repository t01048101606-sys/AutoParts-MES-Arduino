...
def setup_page(title: str):
    st.set_page_config(page_title=f"Auto Parts MES - {title}", layout="wide")
    st.markdown(
        """
        <style>
        .kpi-card {
        ...
        .kpi-red    { background: linear-gradient(135deg, #B0392B, #D9694F); }
        ...
        .alert-danger  { background: #FDECEA; border-color: #B0392B; color: #7A2A10; }
        ...
        .dot-shipment { background: #B0392B; }
        .timeline-tag {
            font-size: 11px;
            font-weight: 700;
            padding: 2px 8px;
        ...
        .tag-shipment { background: #B0392B; }
        </style>
        """,
...
def require_role(*allowed_roles: str):
    """로그인 여부와 역할(role)을 함께 확인한다.
    각 페이지 파일에서 setup_page() 다음 줄에 require_role("ADMIN", "OPERATOR")
    처럼 허용할 역할을 나열해서 호출한다."""
    if not st.session_state.get("logged_in", False):
        st.warning("로그인이 필요합니다. 홈 화면에서 먼저 로그인해주세요.")
        st.page_link("app.py", label="홈으로 이동")
        st.stop()
...
def kpi_row(items: list[tuple[str, str, str]], columns: int = 3):
    """KPI 카드를 그리드로 그린다. items는 (라벨, 값, css_class) 튜플 리스트.
    css_class: kpi-red / kpi-blue / kpi-green / kpi-purple / kpi-gold / kpi-teal"""
