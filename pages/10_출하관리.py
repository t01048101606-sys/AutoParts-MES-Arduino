...
with tab_register:
    ...
    available_lots = queries.finished_lots_with_balance()
    ...
    if not available_lots:
        ...
        customers = queries.active_partners_for_select("CUSTOMER")
        customer_options = {"(선택 안 함)": None}
        for c in customers:
            customer_options[c["partner_name"]] = c["partner_id"]

        col1, col2 = st.columns(2)
        shipment_no = col1.text_input(
            "출하번호", value=f"SHP-{date.today().strftime('%Y%m%d')}-{next_shipment_id:04d}"
        )
        shipment_date = col2.date_input("출하일자", value=date.today())
        customer_label = st.selectbox("출하처(고객사)", list(customer_options.keys()))
        ...
        if st.button("출하 등록", type="primary"):
            data = ShipmentRegistration(
                ...
                partner_id=customer_options[customer_label],
...
with tab_history:
    ...
    keyword = col1.text_input("출하번호 또는 고객사 검색")
    ...
    if not history_df.empty:
        ...
        shipment_id = int(history_df[history_df["shipment_no"] == selected_no].iloc[0]["shipment_id"])
