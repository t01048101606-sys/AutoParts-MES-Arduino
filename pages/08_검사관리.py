...
with tab_register:
    lot_type_label = st.radio(
        "검사 대상", ["원자재 입고 (RECEIPT)", "반제품 (WIP)", "완제품 (FINISHED)"], horizontal=True
    )
    lot_type = {"원자재 입고 (RECEIPT)": "RECEIPT", "반제품 (WIP)": "WIP", "완제품 (FINISHED)": "FINISHED"}[
        lot_type_label
    ]
    ...
    if not uninspected:
        st.info("검사 대상 LOT가 없습니다.")
    else:
        lot_options = {
            f"{lot['lot_no']} | {lot['item_name']} | 수량 {lot['qty']:,.0f}": lot for lot in uninspected
        }
...
with tab_history:
    ...
    type_filter = col3.selectbox("검사 유형", ["전체", "RECEIPT", "WIP", "FINISHED"])
...
with tab_stats:
    show_dataframe(queries.defect_rate_by_item(), "검사 이력이 없습니다.")
...
with tab_reason:
    ...
    with st.form("reason_code_form"):
        new_reason_code = st.text_input("사유 코드 (예: DIM_OUT)")
        new_reason_name = st.text_input("사유명 (예: 치수 불량)")
        reason_submitted = st.form_submit_button("사유 코드 등록", type="primary")

    if reason_submitted:
        data = DefectReasonCodeRegistration(reason_code=new_reason_code, reason_name=new_reason_name)
        try:
    ...
    if not reason_df.empty:
        ...
        if st.button(f"{toggle_row['reason_code']} {'비활성화' if new_status == 'N' else '다시 활성화'}"):
            try:
